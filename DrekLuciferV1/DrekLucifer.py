...
class sender(threading.Thread):
    ...
    def request(self):
        ...
        req = urllib.request.Request(self.url, data, self.headers, self.accept)
        urllib.request.urlopen(req)
        print(Fore.GREEN+'[TINO] SSL-Connected: '+Fore.CYAN+'%s\r\n' % self.url) 
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                sys.stdout.write(Fore.RED+'[Lucifer] SSL-Connect: '+Fore.BLUE+'fail or timeout !\n')
                sys.exit(0)
...
"""      
        ...
        try:
            ...
            url = str(input(f"{Fore.RED}[Lucifer]   Input URL/Target: {Fore.RESET}"))
        except:
            os.system('cls' if os.name == 'nt' else 'clear')
			#url = sys.argv[1]
            url = str(input(f"{Fore.RED}│   Input URL/Target: {Fore.RESET}"))
        else:
            try:
                ...
                num_threads = str(input(f"{Fore.RED}[Lucifer]   Numbers Threads: {Fore.RESET}"))
                if num_threads == '':
